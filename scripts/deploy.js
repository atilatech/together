...
task("deploy:rentableNFT", "Deploys the RentableNFT contract")
.addParam("chainId", "The Chain ID of the blockchain where this contract will be deployed.")
.setAction(async function (taskArguments, hre) {
  ...
  let RentableNFT;
  if (account) {
    RentableNFT = await hre.ethers.getContractFactory("RentableNFT", account);
  } else {
    RentableNFT = await hre.ethers.getContractFactory("RentableNFT");
  }
  let rentableNFT;

  if (gasPrice) {
    rentableNFT = await RentableNFT.deploy({gasPrice});
  } else {
    rentableNFT = await RentableNFT.deploy();
  }
  await rentableNFT.deployed();


  availableChains[chainId].RENTABLE_NFT_ADDRESS = rentableNFT.address.toLowerCase();
  fs.writeFileSync(chainConfigFilePath, JSON.stringify(availableChains, null, 4));

  const chainConfig = availableChains[chainId];
  console.log("\x1b[32m%s\x1b[0m", `RentableNFT deployed to ${chainConfig.CHAIN_NAME} (${chainConfig.NETWORK_NAME}): ${chainConfig.RENTABLE_NFT_ADDRESS}`);
  console.log("\x1b[32m%s\x1b[0m", `View in block explorer: ${chainConfig.BLOCK_EXPLORER_URL}/address/${chainConfig.RENTABLE_NFT_ADDRESS}`);
